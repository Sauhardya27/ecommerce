...
export default function Interests() {
  ...
  const fetchInterests = useCallback(async (page: number) => {
    try {
      setLoading(true);
      setError(null);
    ...
    try {
      ...
      setInterests(prevInterests =>
        prevInterests.map(interest =>
          interest.id === id ? { ...interest, selected } : interest
        )
      );
      ...
      console.error('Failed to update interest:', err instanceof Error ? err.message : err);
    }
  };
  ...
  useEffect(() => {
    void fetchInterests(currentPage);
  }, [currentPage, fetchInterests]);
